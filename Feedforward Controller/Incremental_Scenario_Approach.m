...
% Index counters used in complexity calculations
...
while (complexity > j) && (j< M*u_length)
    ...
    if j==0
        ...
        % The uncertainty vector gets new scenarios
        maxv=zeros(M,1);
    ...
    for p=0: M*x_length-1
        ...
        if (X_max(p+1,1) < g_t_max(p+1,1)+1e-3) && (X_max(p+1,1) > g_t_max(p+1,1)-1e-3)
